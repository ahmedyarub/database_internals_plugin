...
class TableStatsToolWindowFactory : ToolWindowFactory {
    ...
    private fun getContent() = JBPanel<JBPanel<*>>().apply {
        add(tableLabel)
    }

    private fun createTableSelectionListener() {
        // FIXME what if we have more than one project?
    ...
    private fun getCurrentTableName(): String? {
        val dv = DatabaseView.getDatabaseView(ProjectManager.getInstance().openProjects.first())
        val tableElement = dv.selectedElements.first()?.extractObject()
        ...
        return MyBundle.message("defaultLabel")
    }
